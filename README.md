...
| `ORIGIN_HTTP_RULES`           | `{}`                           | Engine       | Optional JSON of request mutations applied when fetching origins, keyed by customer id then by regex matched against the origin URL. The only current operation is `addHeader`. Values are typically secrets and should be injected accordingly. When multiple regexes match, headers are merged with later entries winning. Example: `{"2":{"^https://fraser-staging.*":{"addHeader":{"Auth-Bypass-Key":"..."}}}}`.       |
...
>
> * Backslashes in regexes must be doubled in the JSON, as JSON string escaping consumes one level: write `"^https://customer-url\\.com.*"` to match a literal dot.
> * When set in a `.env` file, wrap the whole value in **single** quotes and keep it on one line — unquoted values are truncated at the first `#`, and double quotes trigger backslash-unescaping that corrupts regex escapes.
> * In a docker-compose `environment:` block, escape any `$` (e.g. a regex end anchor) as `$$` to avoid compose variable interpolation.
